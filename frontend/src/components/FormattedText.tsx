    ...
    <div className={`formatted-text ${className}`}>
      {structured.map((item, index) => {
        switch (item.type) {
          case 'heading':
            return (
              <h1 key={index} className="text-xl font-bold text-slate-900 mb-3 mt-2 border-b border-slate-200 pb-2">
                {item.content}
              </h1>
            );
          ...
          case 'subheading':
            return (
              <h2 key={index} className={`font-semibold text-slate-800 mb-2 mt-4 ${
          ...
          case 'bullet':
            return (
              <div key={index} className="flex items-start space-x-2 mb-2 text-slate-700">
